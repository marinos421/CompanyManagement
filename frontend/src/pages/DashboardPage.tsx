...
const DashboardPage = () => {
  ...
  const fetchData = async () => {
    try {
      // Καλούμε και τα δύο Services παράλληλα για ταχύτητα
  ...
  // --- Calculations ---
  // Μόνο Completed συναλλαγές για τα οικονομικά
  const completedTrans = transactions.filter(t => t.status === "COMPLETED");
  
  const totalIncome = completedTrans
    .filter(t => t.type === "INCOME")
    .reduce((sum, t) => sum + t.amount, 0);

  const totalExpense = completedTrans
    .filter(t => t.type === "EXPENSE")
    .reduce((sum, t) => sum + t.amount, 0);

  const netBalance = totalIncome - totalExpense;

  // Παίρνουμε τις 5 πιο πρόσφατες (το Backend τις στέλνει ήδη ταξινομημένες, άρα παίρνουμε τις 5 πρώτες)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-xl p-6">
            ...
            <div className="space-y-3">
                <Link to="/transactions" className="block w-full bg-slate-700 hover:bg-slate-600 text-white p-3 rounded-lg text-center transition border border-slate-600">
                    + Add New Transaction
                </Link>
                <Link to="/employees" className="block w-full bg-slate-700 hover:bg-slate-600 text-white p-3 rounded-lg text-center transition border border-slate-600">
                    + Register Employee
                </Link>
                <Link to="/profile" className="block w-full bg-slate-700 hover:bg-slate-600 text-white p-3 rounded-lg text-center transition border border-slate-600">
                    Update Company Info
                </Link>
            </div>
            ...
            <div className="mt-8 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                ...
                <p className="text-slate-400 text-sm">You can categorize transactions to see better charts in the Transactions page.</p>
            </div>
        </div>
